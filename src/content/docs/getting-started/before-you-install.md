---
title: "Before you install"
description: "A short pre-flight checklist."
products: ["standard", "enterprise-management", "field-app"]
roles: []
introduced: "1.0.0"
contentType: "concept"
lastReviewed: "2026-07-30"
---

Before installing, confirm:

- You meet the system requirements.
- You can reach the Ignition Gateway **web configuration page**.
- You have the module `.modl` and the Perspective project files (Source + your product's project(s)).
- You have your **network-server API credentials** and **MQTT broker** details.
- **(Enterprise)** your Identity Provider has the **administrator** and **general** roles ready.

Then follow the [Installation overview](/install-upgrade/overview/).
